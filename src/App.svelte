...
{#if connected}<span style="color: greenyellow; font-weight: bold;"
    >Connected</span
  ><br />{/if}
{#if !connected}<span style="color: red; font-weight: bold;">Not Connected</span
  ><br />{/if}
...
>
  {#each msglist as msg}
    <div
      class="msg"
      class:own={msg.user.toString() == user.toString()}
      tabindex="-1"
    >
